...
> ⚠️ When `STS_URL` is configured, **ASR_URL, LLM_URL, and TTS_URL must be commented out**, as AVR Core switches to STS mode.

## How It Works

1. Asterisk streams audio to AVR Core via AudioSocket
2. AVR Core forwards audio to the HumeAI STS service
3. HumeAI processes speech and generates a spoken response in real time
4. Audio is streamed back to AVR Core
5. AVR Core sends audio back to Asterisk
