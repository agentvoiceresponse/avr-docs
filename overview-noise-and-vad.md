...
Learn how **Agent Voice Response (AVR)** intelligently handles **background noise** and **voice activity detection (VAD)** to create natural, responsive, real-time voice conversations.

Voice Activity Detection and ambient noise handling are fundamental components that determine how natural and human-like the caller experience feels. AVR provides a **multi-layered audio pipeline** that works out of the box but is also highly configurable for advanced tuning.

---

## Our Philosophy: Smart Defaults

AVR is designed with **smart, adaptive defaults** so that most installations work perfectly without manual configuration.
...
By default, AVR activates **VAD automatically** and applies **light ambient noise** mixing to improve the listening experience.

---

## Ambient Noise

AVR Core supports optional **background sound blending** to make AI conversations sound more natural and reduce the “dead air” effect between responses.
...
AVR Core includes an integrated **Silero VAD** engine that continuously analyzes the audio stream to detect when a user is speaking or silent.
This enables:
...
For advanced tuning, AVR exposes several `VAD_*` environment variables based on **Silero VAD**. These allow fine-grained control over speech sensitivity, latency, and robustness.
...
* 🔗 [Silero VAD Repository](https://github.com/agentvoiceresponse/avr-vad)
* 🧩 [Understanding AVR Core](https://wiki.agentvoiceresponse.com/en/understanding-avr-core)
* 🔊 [Ambient Sounds Examples](https://github.com/agentvoiceresponse/avr-infra/tree/main/ambient_sounds)